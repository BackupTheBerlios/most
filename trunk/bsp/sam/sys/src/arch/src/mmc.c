...
disc_install(MFS_descriptor_t *dir)
{
    ...
    MFS_descriptor_t *desc = MFS_block_create (mmc, "d1_p1", &mmc_desc_op, &mmc_block_op, (MFS_represent_t *) &d1_p1, MFS_DISK);
	mmc_init(desc, SAM_PARTITION_1_START, SAM_PARTITION_1_END, SAM_MMC_BLOCK_SIZE);

	desc = MFS_block_create (mmc, "d1_p2", &mmc_desc_op, &mmc_block_op, (MFS_represent_t *) &d1_p2, MFS_DISK);
	mmc_init(desc, SAM_PARTITION_2_START, SAM_PARTITION_2_END, SAM_MMC_BLOCK_SIZE);

	desc = MFS_block_create (mmc, "d1_p3", &mmc_desc_op, &mmc_block_op, (MFS_represent_t *) &d1_p3, MFS_DISK);

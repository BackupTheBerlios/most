...
config_ip (void)
{
    NET_netif_set_ipaddr (&SAM_eth0, &CLI_config.ip_addr);
    NET_netif_set_netmask (&SAM_eth0, &CLI_config.netmask);
    NET_netif_set_gateway (&SAM_eth0, &CLI_config.gateway);
    return ACE_OK;
}

extern void
SAM_config_install (void)
{

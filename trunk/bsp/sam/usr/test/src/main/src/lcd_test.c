...
static void display_init(void)
{
	if (0 > DPY_nokia6100_spi_init ((DEV_spi_dev_t *)&SAM_display)){
	    USO_kputs (USO_LL_ERROR, "LCD spi init failed!\n");
	}
...
static void display_logo(void)
{
	...
	DPY_nokia6100_put_string ("M O S T", 8, 37, DPY_NOKIA6100_LARGE, DPY_NOKIA6100_RED, DPY_NOKIA6100_GREEN);
	DPY_nokia6100_put_string (SAM_APPLICATION, 102, 30, DPY_NOKIA6100_SMALL, DPY_NOKIA6100_RED, DPY_NOKIA6100_GREEN);
	DPY_nokia6100_put_string (ACE_MOST_VERSION, 115, 30, DPY_NOKIA6100_SMALL, DPY_NOKIA6100_RED, DPY_NOKIA6100_GREEN);
...
static void display_running(void)
{
	...
	USO_sleep (ACE_MSEC_2_TICKS(25));
}

static void display_menu(char *s)
{
	DPY_nokia6100_set_rectangle (0, 0, 131, 131, DPY_NOKIA6100_FILL, DPY_NOKIA6100_BLUE);
	DPY_nokia6100_put_string (s, 10, 10, DPY_NOKIA6100_LARGE, DPY_NOKIA6100_RED, DPY_NOKIA6100_WHITE);
}


static void display_value(char *s)
{
	DPY_nokia6100_set_rectangle (40, 10, 50, 120, DPY_NOKIA6100_FILL, DPY_NOKIA6100_BLUE);
	DPY_nokia6100_put_string (s, 40, 10, DPY_NOKIA6100_SMALL, DPY_NOKIA6100_WHITE, DPY_NOKIA6100_BLUE);
}

static char* print_value(unsigned long v, char *text)
{
	static char *s = "ERR";
	static char buf[30];
...
static void play_mic(void)
{
	static long print_delay = 1000;
	ACE_u32_t delay = DEV_adc_get(&adc, DEV_ADC_CHN_6);
	ACE_u32_t audio = DEV_adc_get(&adc, DEV_ADC_CHN_7);
	DEV_pwm_set(&pwm, audio);
	DEV_cpudelay (ACE_USEC_2_LOOPS(delay));

	if (print_delay-- <= 0){
		print_delay = 1000;
		char *s = print_value(delay, "us");
		display_value(s);
	}
}
...
static void play_audio(void)
{
	...
	DEV_pwm_set(&pwm, DEV_pwm_sin[sin]);
	...
	if (v < 1){
		...
		USO_sleep (ACE_MSEC_2_TICKS(20/v));
	} else {
		DEV_cpudelay (ACE_USEC_2_LOOPS(20000/v));
	}

	if (DEV_pwm_sin[sin] < 0)
	{
		char *s = print_value(v, "hz");
		display_value(s);
		sin = 0;
	}
	sin++;
...
static void get_temp(void) {
	...
	USO_sleep (ACE_MSEC_2_TICKS(150));
}

static ACE_bool_t change_state(USO_node_t *e, enum lcd_state state)
{
	lcd_state = state;
...
static ACE_bool_t send_event(USO_node_t *e, enum SAM_event event)
{
	e->data = event;
	return USO_slots_put_event(&SAM_event_slots, NULL, e, FALSE);
}

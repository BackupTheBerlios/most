...
static void write_command (ACE_u32_t cmd)
{
	DEV_spi_exchange (spi, (cmd & ~0x0100), NULL); // BIT9 = 0 -> command
}

static void write_data (ACE_u32_t data)
{
	DEV_spi_exchange (spi, (data | 0x0100), NULL); // BIT9 = 1 -> data
}

static void set_column_address (int x, int y)
{
	// column address set
	write_command (CASET);
	write_data (x);
...
static void set_page_address (int x, int y)
{
	// page address set
	write_command (PASET);
	write_data (x);

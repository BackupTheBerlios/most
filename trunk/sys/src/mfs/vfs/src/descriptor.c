...
{
	if (desc->operations->open != NULL) {desc->operations->open(desc->entry);}
}
...
{
	if (desc->operations->close != NULL) {desc->operations->close(desc->entry);}
	return desc->parent;
}
...
{
	if (desc->operations->info != NULL ) {desc->operations->info(desc->entry);}
}

...
static void
stream_open (MFS_entry_t * entry)
{
    MFS_stream_t *stream = (MFS_stream_t *) entry;
    if (stream->operations->open != NULL)
    {
        stream->operations->open (stream);
    }
}

static void
stream_close (MFS_entry_t * entry)
{
    MFS_stream_t *stream = (MFS_stream_t *) entry;
    if (stream->operations->close != NULL)
    {
...
stream_info (MFS_entry_t * entry)
{
    ...
    if (stream->operations->info != NULL)
    {
        stream->operations->info (stream);
    }
}
...
{
    if (stream->operations->flush != NULL)
    {
        stream->operations->flush (stream);
    }
}

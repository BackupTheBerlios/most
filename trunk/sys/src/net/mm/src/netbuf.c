...
{
    USO_buf_pool_init (&net_pool, net_bufs, NET_BUF_COUNT, sizeof (NET_netbuf_t));
    USO_buf_pool_init (&trans_pool, trans_bufs, NET_TRANSBUF_COUNT, sizeof (struct trans_buf));
}
...
NET_netbuf_free (NET_netbuf_t * buf)
{
    ...
    while (buf != NULL)
    {
        ...
        case NET_BUF_POOL:
            ...
            break;
        case NET_BUF_FREE:
            USO_log_printf (USO_LL_ERROR, "netbuf free\n");
            break;
        default:
            USO_log_printf (USO_LL_ERROR, "netbuf inval\n");
            break;
        }
...
{
    ACE_bool_t ok = FALSE;
    if ((buf->index + inc) >= buf->data && (buf->index + inc) <= (buf->data + buf->size))
    {
        buf->index += inc;
        buf->len -= inc;
        ok = TRUE;

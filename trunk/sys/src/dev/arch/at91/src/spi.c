...
static int	DEV_at91_spi_exchange	(DEV_spi_dev_t *dev,
									 ACE_u32_t out,
									 ACE_u32_t* in);
...
DEV_at91_spi_bus_init (DEV_at91_spi_bus_t *at91, int devices)
{
	...
	if (0 == bus->number)
	{
		...
		p_regs = (at91->devices | AT91C_PA16_SPI0_MISO | AT91C_PA17_SPI0_MOSI | AT91C_PA18_SPI0_SPCK);
	}
	else if (1 == bus->number)
	{
		at91->id = 	AT91C_ID_SPI1;
		at91->base = AT91C_BASE_SPI1;
		p_regs = (at91->devices | AT91C_PA24_SPI1_MISO | AT91C_PA23_SPI1_MOSI | AT91C_PA22_SPI1_SPCK);
	}
	else
		return (-1);
...
DEV_at91_spi_dev_config (DEV_spi_dev_t *dev)
{
	...
	at91->bus->base->SPI_CSR[dev->number] =
		(AT91C_SPI_DLYBCT & (dev->delay_b_ct << 24)) |      // delay between consecutive transfers

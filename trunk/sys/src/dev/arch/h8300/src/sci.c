...
extern void
DEV_h8300_sci_open (int port, const struct DEV_serial_settings *settings)
{
    ...
    p->SMR =
        ...
        stopbittable[settings->stop] | wakeuptable[0];
    p->BRR = baudratetable[settings->baud].brr;

    // Wait for the baud rate generator to stabilize (the time to send one 
    // bit).
    DEV_cpudelay (baudratetable[settings->baud].delay * ACE_USEC_2_LOOPS(1000));
...
extern void
DEV_h8300_sci_start (int port, DEV_serial_t * serial)
{
    int c;
    sci_reg_t *const p = ports[port];
...
extern void
DEV_h8300_sci_txe (int port, DEV_serial_t * serial)
{
    int c;
    sci_reg_t *const p = ports[port];
...
extern void
DEV_h8300_sci_txi (int port, DEV_serial_t * serial)
{
    int c;
    sci_reg_t *const p = ports[port];
...
extern void
DEV_h8300_sci_rxi (int port, DEV_serial_t * serial)
{
    char c;
    sci_reg_t *const p = ports[port];
...
extern void
DEV_h8300_sci_eri (int port, DEV_serial_t * serial)
{
    sci_reg_t *const p = ports[port];

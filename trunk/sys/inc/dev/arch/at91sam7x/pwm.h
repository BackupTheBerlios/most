...
/**
 * Sets the period and duty value used by a PWM channel. This function writes directly
 * to the CPRD register if the channel is disabled; otherwise, it uses the
 * update register CUPD and only updates the period.
 * @param channel : Channel number.
 * @param period : Period value.
 * @param duty : Duty cycle value.
 */
void DEV_at91_PWM_set_period (unsigned char channel, unsigned short period, unsigned short duty);
 ...
 * period.
 * @param channel : Channel number.
 * @param period : this value is not used, just for compatibility for dev with delay and pulse values.
 * @param duty : Duty cycle value.
 */
void DEV_at91_PWM_set_duty_cycle (unsigned char channel, unsigned short period, unsigned short duty);

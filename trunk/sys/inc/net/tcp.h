...
err_t tcp_write (struct tcp_pcb *pcb,
                 const void *dataptr, u16_t len, u8_t copy);

/*
 * It is also possible to call these two functions at the right intervals
 * (instead of calling tcp_tmr()). 
 */
...
#define TCP_MSL 60000           /* The maximum segment lifetime in
                                 * microseconds */
...
#define TF_GOT_FIN   0x20       /* Connection was closed by the remote
                                 * end. */
...
err_t tcp_enqueue (struct tcp_pcb *pcb,
                   void *dataptr,
                   u16_t len,
                   u8_t flags, u8_t copy, u8_t * optdata, u8_t optlen);
 ...
 * The TCP PCB lists. 
 */
extern struct tcp_pcb_listen *tcp_listen_pcbs;  /* List of all TCP PCBs in 
                                                 * LISTEN state. */
extern struct tcp_pcb *tcp_active_pcbs; /* List of all TCP PCBs that are
                                         * in a state in which they accept 
                                         * or send data. */
extern struct tcp_pcb *tcp_tw_pcbs;     /* List of all TCP PCBs in
                                         * TIME-WAIT. */

extern struct tcp_pcb *tcp_tmp_pcb;     /* Only used for temporary
                                         * storage. */

/*
 * Axoims about the above lists: 1) Every TCP PCB that is not CLOSED is
 * in one of the lists. 2) A PCB is only in one of the lists. 3) All PCBs
 * in the tcp_listen_pcbs list is in LISTEN state. 4) All PCBs in the
 * tcp_tw_pcbs list is in TIME-WAIT state. 
 */

/*
 * Define two macros, TCP_REG and TCP_RMV that registers a TCP PCB with a
 * PCB list or removes a PCB from a list, respectively. 
 */
#ifdef LWIP_DEBUG
